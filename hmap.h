...
* using a probing collision solving. The hash map stores elements with 
* elem_size bytes each identified by a key with key_size bytes and can 
* store up to map_size/(elem_size+key_size).
* 	There is a reserved key/elem combination that is used to mark empty and
* deleted map positions. The reserved key/elem consist in all the bits in 
* the elem and the key been only 1s or only 0s.
...
* 	@param map Pointer to a hmap structure from where the elem is going to
* be removed.
* 	@param key Pointer to a key that identifies the element to be removed 
* 	@param elem Pointer to which the removed element will be writen before
* removal.
* 	@return Returns SUCCESS if the element was found and the removal was 
...
* 	@param map Pointer to a hmap structure from where the element is going
* to be searched.
